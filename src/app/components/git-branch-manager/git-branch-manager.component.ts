...
export class GitBranchManagerComponent {
  ...
  repoPath = signal('~/Documents/workspace/informed-backend/tmp/informed-program-config');
  ...
  presetBranches = [
    ...
    'release/vr-mivrs-20260601',
    'release/vr-okvrs-20260601',
    'release/vr-nhvr-20260601',
    'release/vr-tnvrs-20260601',
    'release/vr-alvrs-20260601',
    'release/vr-wvdrs-20260601',
  ];
  ...
  addPreset(branch: string): void {
    ...
    this.branches.update(prev => [...prev, {
      sourceBranch: branch,
  ...
  removeBranch(sourceBranch: string): void {
    this.branches.update(prev => prev.filter(b => b.sourceBranch !== sourceBranch));
  }

  toggleSelect(sourceBranch: string): void {
    this.branches.update(prev =>
      prev.map(b => b.sourceBranch === sourceBranch ? { ...b, selected: !b.selected } : b),
    );
  }
  ...
  deselectAll(): void {
    this.branches.update(prev => prev.map(b => ({ ...b, selected: false })));
  }

  refreshBranchNames(): void {
    this.branches.update(prev =>
      prev.map(b => ({ ...b, newBranch: this.buildNewBranchName(b.sourceBranch) })),
    );
  }
  ...
  copyScript(): void {
    ...
    navigator.clipboard.writeText(script).then(() => this.snackbar.success('Script copied to clipboard.'));
  }
  ...
  runScript(): void {
    ...
    fetch('http://localhost:3333/api/run-script', {
      ...
      .then(res => res.json())
      .then(data => {
        this.scriptOutput.set(data.stdout + (data.stderr ? '\n' + data.stderr : ''));
      ...
      .catch(err => {
        this.scriptOutput.set('Failed to connect to script runner.\nMake sure the server is running: node server.js');
        this.scriptSuccess.set(false);
        this.isRunning.set(false);
        this.snackbar.error('Cannot connect to script runner (localhost:3333).');
      });
  }
  ...
  private buildScript(): string {
    ...
    if (this.applyMode() === 'stash') {
      ...
      lines.push('  echo "ERROR: No stash found. Stash your changes first with: git stash"');
      lines.push('  exit 1');
      lines.push('fi');
      lines.push('');
    }
